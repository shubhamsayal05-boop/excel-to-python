...
def parse_odriv_from_excel(file_obj):
    ...
    # We need: Current Status cols (P1/P2/P3 dots), the "Lowest Events"
    # divider cols, and ALL vehicle percentage columns in between.
    driv_current_col = None   # first "Current Status" → Driv P1
    resp_current_col = None   # second "Current Status" → Resp P1
    driv_lowest_col = None    # "Drivability Lowest Events" column
    resp_lowest_col = None    # "Responsiveness Lowest Events" column

    # Known non-vehicle labels in row 21
    _SKIP_LABELS = {
        "current status", "sopm prediction",
        "driveability index", "drivability index",
        "responsiveness index",
        "drivability lowest events", "driveability lowest events",
        "responsiveness lowest events",
    }

    # First pass: find structural anchors
    for col_idx in range(1, ws.max_column + 1):
        ...
        if "drivability lowest" in val_lower or "driveability lowest" in val_lower:
            driv_lowest_col = col_idx

        if "responsiveness lowest" in val_lower:
            resp_lowest_col = col_idx

    # Fallback: if we couldn't find required columns, bail out
    if driv_current_col is None:
        wb.close()
        return None

    # Derive P1/P2/P3 offsets
    ...
    # --- Discover ALL vehicle columns ---
    # Vehicle columns sit between the last P3 (or SOPM Prediction P3) and
    # the "Lowest Events" divider.  A vehicle column is any column in row 21
    # that has a non-empty value which is NOT a known structural label.
    def _find_vehicle_cols(start_after_col, end_before_col):
        """Return list of (col_idx, vehicle_name) between two boundary cols."""
        vehicles = []
        if start_after_col is None or end_before_col is None:
            return vehicles
        for c in range(start_after_col + 1, end_before_col):
            cell_val = ws.cell(row=21, column=c).value
            if cell_val is not None:
                name = str(cell_val).strip()
                if name and name.lower() not in _SKIP_LABELS and not name.startswith("="):
                    vehicles.append((c, name))
        return vehicles

    driv_vehicle_cols = _find_vehicle_cols(driv_p3_col, driv_lowest_col)
    resp_vehicle_cols = _find_vehicle_cols(resp_p3_col, resp_lowest_col) if resp_p3_col and resp_lowest_col else []

    # Build ordered list of unique vehicle names (Drivability is authoritative;
    # Responsiveness typically mirrors it).
    all_vehicles = []
    seen = set()
    for _, vname in driv_vehicle_cols:
        if vname not in seen:
            all_vehicles.append(vname)
            seen.add(vname)
    for _, vname in resp_vehicle_cols:
        if vname not in seen:
            all_vehicles.append(vname)
            seen.add(vname)

    # Build column-index lookup dicts: vehicle_name -> col_idx
    driv_col_map = {vname: cidx for cidx, vname in driv_vehicle_cols}
    resp_col_map = {vname: cidx for cidx, vname in resp_vehicle_cols}

    # --- Backwards-compatible: pick default tested / target ---
    # Tested: first vehicle (usually the project vehicle).  If the HOME
    # sheet has a project name, prefer that for the display label but
    # verify it matches one of the discovered vehicles.
    ...
    if raw_tested is not None:
        ...
        if not tested_str.startswith("="):
            home_tested = tested_str
    if not home_tested and "HOME" in wb.sheetnames:
        home_val = wb["HOME"].cell(row=9, column=3).value
        if home_val:
            home_tested = str(home_val).strip()

    # Default tested = first vehicle whose name matches home_tested, or
    # simply the first vehicle in the list.
    tested_car = all_vehicles[0] if all_vehicles else "Tested Vehicle"
    if home_tested:
        for v in all_vehicles:
            if home_tested in v or v in home_tested:
                tested_car = v
                break

    # Default target = second vehicle if available, otherwise first
    if len(all_vehicles) >= 2:
        target_car = all_vehicles[1]
    elif all_vehicles:
        target_car = all_vehicles[0]
    else:
    ...
    for row_idx in range(data_start, ws.max_row + 1):
        ...
        if is_section:
            # Read per-vehicle averages for this section
            driv_vehicle_avgs = {}
            resp_vehicle_avgs = {}
            for vname in all_vehicles:
                dcol = driv_col_map.get(vname)
                rcol = resp_col_map.get(vname)
                driv_vehicle_avgs[vname] = _to_float(
                    ws.cell(row=row_idx, column=dcol).value if dcol else None
                )
                resp_vehicle_avgs[vname] = _to_float(
                    ws.cell(row=row_idx, column=rcol).value if rcol else None
                )
            sections.append({
                "name": col_b_str,
                "driv_tested_avg": driv_vehicle_avgs.get(tested_car, 0.0),
                "driv_target_avg": driv_vehicle_avgs.get(target_car, 0.0),
                "resp_tested_avg": resp_vehicle_avgs.get(tested_car, 0.0),
                "resp_target_avg": resp_vehicle_avgs.get(target_car, 0.0),
                "driv_vehicle_avgs": driv_vehicle_avgs,
                "resp_vehicle_avgs": resp_vehicle_avgs,
            })
        ...
        # Read per-vehicle percentages
        driv_pcts = {}
        resp_pcts = {}
        for vname in all_vehicles:
            dcol = driv_col_map.get(vname)
            rcol = resp_col_map.get(vname)
            driv_pcts[vname] = _to_float(
                ws.cell(row=row_idx, column=dcol).value if dcol else None
            )
            resp_pcts[vname] = _to_float(
                ws.cell(row=row_idx, column=rcol).value if rcol else None
            )
        ...
        op_data = {
            ...
            "driv_tested": driv_pcts.get(tested_car, 0.0),
            "driv_target": driv_pcts.get(target_car, 0.0),
            ...
            "resp_tested": resp_pcts.get(tested_car, 0.0),
            "resp_target": resp_pcts.get(target_car, 0.0),
            # Per-vehicle data for multi-vehicle support
            "driv_pcts": driv_pcts,
            "resp_pcts": resp_pcts,
        }
        operations.append(op_data)
...
def remap_odriv_vehicles(sheet1_data, target_car, tested_car):
    """Remap the per-operation tested/target percentages based on user selection.

    When the ODRIV file contains multiple vehicles, each operation stores
    per-vehicle percentages in ``driv_pcts`` / ``resp_pcts`` dicts.  This
    function updates the ``driv_tested``, ``driv_target``, ``resp_tested``,
    ``resp_target`` fields (used by the evaluation engine) to reflect the
    user's choice of target and tested vehicle.

    Also updates section-level averages and the top-level car name fields.

    Args:
        sheet1_data: dict from ``parse_odriv_from_excel``.
        target_car:  name of the user-selected target vehicle.
        tested_car:  name of the user-selected tested vehicle.
    """
    sheet1_data["target_car"] = target_car
    sheet1_data["tested_car"] = tested_car

    for op in sheet1_data.get("operations", []):
        driv_pcts = op.get("driv_pcts", {})
        resp_pcts = op.get("resp_pcts", {})
        op["driv_tested"] = driv_pcts.get(tested_car, 0.0) or 0.0
        op["driv_target"] = driv_pcts.get(target_car, 0.0) or 0.0
        op["resp_tested"] = resp_pcts.get(tested_car, 0.0) or 0.0
        op["resp_target"] = resp_pcts.get(target_car, 0.0) or 0.0

    for sec in sheet1_data.get("sections", []):
        driv_avgs = sec.get("driv_vehicle_avgs", {})
        resp_avgs = sec.get("resp_vehicle_avgs", {})
        if driv_avgs:
            sec["driv_tested_avg"] = driv_avgs.get(tested_car, 0.0) or 0.0
            sec["driv_target_avg"] = driv_avgs.get(target_car, 0.0) or 0.0
        if resp_avgs:
            sec["resp_tested_avg"] = resp_avgs.get(tested_car, 0.0) or 0.0
            sec["resp_target_avg"] = resp_avgs.get(target_car, 0.0) or 0.0
...
def generate_red_comments(sheet1_data, heatmap_df, odriv_details,
    ...
    **Drivability**, the base ``"<sheet>"`` key is used.
    ...
    for op in operations:
        ...
        # Skip operations where both P1 fields are N/A — their final status
        # is always N/A regardless of AVL, so no RED comment applies.
        if driv_p1 == "N/A" and resp_p1 == "N/A":
            continue
        ...
        # and the sheet-level P1 indicates RED.
        ...
        if all_parts:
            new_comment = " | ".join(all_parts)
            existing = comments.get(op_code)
            if existing is None:
                comments[op_code] = new_comment
            else:
                # Multiple operations can share the same op_code (e.g.
                # "Vehicle Stop" and "Vehicle Stop - Cold").  Never
                # overwrite a P1-RED comment with a weaker AVL<7 one.
                existing_has_p1 = "Red P1" in existing
                new_has_p1 = "Red P1" in new_comment
                if new_has_p1 or not existing_has_p1:
                    comments[op_code] = new_comment
