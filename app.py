...
def sheet1_input_page():
    ...
    **Recommended:** Upload the Excel file directly so dot colors (green / yellow / red)
    ...
    if input_method == "Upload Excel File (recommended)":
        ...
        if uploaded_file and st.button("🔄 Process Excel File", key="process_sheet1_excel"):
            ...
            if parsed:
                ...
                all_v = parsed.get("all_vehicles", [])
                vehicle_msg = ""
                if all_v:
                    vehicle_msg = (
                        f"\n\n**Vehicles found ({len(all_v)}):** "
                        + ", ".join(all_v)
                    )
                st.success(
    ...
    if "sheet1_data" in st.session_state:
        ...
        # --- Vehicle selection dropdowns (when multiple vehicles exist) ---
        if all_vehicles and len(all_vehicles) >= 2:
            st.subheader("Vehicle Selection")
            st.caption(
                "Select which vehicles to use as **Tested** and **Target** "
                "for evaluation.  The data table below shows all vehicles."
            )
            vcol1, vcol2 = st.columns(2)
            with vcol1:
                current_tested = data.get("tested_car", all_vehicles[0])
                tested_idx = (
                    all_vehicles.index(current_tested)
                    if current_tested in all_vehicles else 0
                )
                selected_tested = st.selectbox(
                    "Tested Vehicle:",
                    all_vehicles,
                    index=tested_idx,
                    key="odriv_tested_select",
                )
            with vcol2:
                current_target = data.get("target_car", all_vehicles[-1])
                target_idx = (
                    all_vehicles.index(current_target)
                    if current_target in all_vehicles else
                    min(1, len(all_vehicles) - 1)
                )
                selected_target = st.selectbox(
                    "Target Vehicle:",
                    all_vehicles,
                    index=target_idx,
                    key="odriv_target_select",
                )

            # Re-map if selection changed
            if (selected_tested != data.get("tested_car") or
                    selected_target != data.get("target_car")):
                remap_odriv_vehicles(data, selected_target, selected_tested)

        st.subheader("Parsed Operations")

        ops = data["operations"]
        if ops:
            html = _build_sheet1_html(
                ops, data.get("sections", []),
        ...
        if data["sections"]:
            ...
            # Drop internal per-vehicle dicts from display
            display_cols = [c for c in sections_df.columns
                           if c not in ("driv_vehicle_avgs", "resp_vehicle_avgs")]
...
def heatmap_view_page():
    ...
    # Remove the AVL-DRIVE Rating row (10000000) — it is not evaluated
    display_df = display_df[display_df["Op Code"] != 10000000].reset_index(drop=True)
...
def evaluation_results_page():
    ...
    # Car selection — use dropdown when multiple vehicles are available
    st.subheader("Vehicle Selection")
    all_vehicles = sheet1_data.get("all_vehicles", [])

    col1, col2 = st.columns(2)
    if all_vehicles and len(all_vehicles) >= 2:
        with col1:
            current_target = sheet1_data.get("target_car", all_vehicles[0])
            target_idx = (
                all_vehicles.index(current_target)
                if current_target in all_vehicles
                else min(1, len(all_vehicles) - 1)
            )
        ...
        with col2:
            current_tested = sheet1_data.get("tested_car", all_vehicles[0])
            tested_idx = (
                all_vehicles.index(current_tested)
                if current_tested in all_vehicles else 0
            )
    ...
    if st.button("🔄 Run Evaluation", type="primary", key="run_eval"):
        with st.spinner("Running evaluation..."):
            # Remap vehicle data if multi-vehicle ODRIV
            if sheet1_data.get("all_vehicles"):
                remap_odriv_vehicles(sheet1_data, target_car, tested_car)
...
def help_page():
    ...
    with st.expander("💬 Auto-Comment Generation", expanded=True):
        st.markdown("""
        ### Auto-generated Comments for RED Status

        When the **full ODRIV Excel file** (`.xlsm`) is uploaded on the
        **Odriv Data Input** page, the tool automatically parses both the
        **Drivability** and **Responsiveness** sections of each detail
        sub-sheet and generates comments for sub-operations with RED P1
        status.

        For **RED P1 Drivability**, events are read from the left-side
        (Drivability) section of the matching detail sheet.  For
        **RED P1 Responsiveness**, events are read from the right-side
        (Responsiveness) section.

        The tool filters for **Priority = 1** and **Event Rating = Red or
        Red+**, considers only **colored cells** (rated criteria), finds the
        event with the **lowest value**, and builds a comment:

        `Red P1 Drivability, Brake Release Bump, GS_RL_0%`

        Comments are displayed in the **HeatMap** view under the
        *Comments* column.
        """)
...
def _build_heatmap_html(df, vehicle_names, target_label, target_vehicle=None, tested_vehicle=None):
    ...
    # Reorder vehicle columns: Target first, Tested last, others in between.
    if target_vehicle and tested_vehicle and target_vehicle != tested_vehicle:
        middle = [c for c in vehicle_cols if c != target_vehicle and c != tested_vehicle]
        ordered = []
...
def _build_sheet1_html(operations, sections, tested_car, target_car,
    ...
    When *all_vehicles* is provided (multi-vehicle ODRIV file), the table
    shows a column for every vehicle under both Drivability and Responsiveness.
    Otherwise falls back to the original 2-column (tested + target) layout.
    """
    # Determine which vehicle names to show as columns
    if all_vehicles and len(all_vehicles) > 0:
        vehicle_names = all_vehicles
    else:
        vehicle_names = [tested_car, target_car]

    n_vehicles = len(vehicle_names)
    driv_colspan = 3 + n_vehicles   # P1 P2 P3 + vehicle cols
    resp_colspan = 3 + n_vehicles
    ...
    r += f'<td class="s1-hdr" colspan="{driv_colspan}">Drivability</td>'
    r += f'<td class="s1-hdr" colspan="{resp_colspan}">Responsiveness</td>'
    r += '</tr>'
    rows_html.append(r)

    # --- Header row 2: P1 P2 P3 + vehicle names ---
    r = '<tr>'
    r += '<td class="s1-sub">P1</td>'
    r += '<td class="s1-sub">P2</td>'
    r += '<td class="s1-sub">P3</td>'
    for vname in vehicle_names:
        r += f'<td class="s1-sub">{_html.escape(vname)}</td>'
    r += '<td class="s1-sub">P1</td>'
    r += '<td class="s1-sub">P2</td>'
    r += '<td class="s1-sub">P3</td>'
    for vname in vehicle_names:
    ...
    for op in operations:
        ...
        if section_name and section_name not in rendered_sections:
            ...
            # Driv vehicle averages
            driv_avgs = sec_info.get("driv_vehicle_avgs", {}) if sec_info else {}
            if driv_avgs:
                for vname in vehicle_names:
                    v = driv_avgs.get(vname, 0)
                    r += f'<td class="s1-pct">{_fmt_score(v) if v else ""}</td>'
            else:
                # Fallback: old 2-column layout
            ...
            r += '<td></td><td></td><td></td>'  # P1 P2 P3
            # Resp vehicle averages
            resp_avgs = sec_info.get("resp_vehicle_avgs", {}) if sec_info else {}
            if resp_avgs:
                for vname in vehicle_names:
                    v = resp_avgs.get(vname, 0)
                    r += f'<td class="s1-pct">{_fmt_score(v) if v else ""}</td>'
            else:
        ...
        # Drivability vehicle percentages
        driv_pcts = op.get("driv_pcts", {})
        if driv_pcts:
            for vname in vehicle_names:
                r += f'<td class="s1-pct">{_fmt_score(driv_pcts.get(vname, 0))}</td>'
        else:
        ...
        # Responsiveness vehicle percentages
        resp_pcts = op.get("resp_pcts", {})
        if resp_pcts:
            for vname in vehicle_names:
                r += f'<td class="s1-pct">{_fmt_score(resp_pcts.get(vname, 0))}</td>'
        else:
